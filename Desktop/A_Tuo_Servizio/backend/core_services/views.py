...
class ProfessionalSearchView(generics.ListAPIView):
    ...
    search_fields = ['business_name', 'user__username', 'user__email', 'tagline', 'description']  # Fields to search in
    ordering_fields = ['user__username', 'business_name', 'years_of_experience']  # Fields to sort by
    ...
    def get_queryset(self):
        ...
        if service:
            queryset = queryset.filter(services__name=service)

        if location:
            queryset = queryset.filter(operating_areas__city=location)

        # Filtering by portfolio
        has_portfolio = self.request.query_params.get('has_portfolio', None)
        if has_portfolio:
            if has_portfolio.lower() == 'true':
                queryset = queryset.filter(portfolio_items__isnull=False).distinct()
            elif has_portfolio.lower() == 'false':
                queryset = queryset.filter(portfolio_items__isnull=True)

        # Filtering by rating (example: rating >= 4)
        rating_min = self.request.query_params.get('rating_min', None)
        if rating_min:
            queryset = queryset.annotate(avg_rating=models.Avg('reviews__rating')).filter(avg_rating__gte=rating_min)

        # TODO: Filtering by availability (more complex, requires a dedicated model/field)
